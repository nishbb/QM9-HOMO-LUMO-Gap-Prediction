...
**CMU 24-788 Introduction to Deep Learning — Mini-Project (Solo, Bonus submission)**
...
├── reproduce.py      # Loads checkpoints → regenerates all figures + Table 1
├── checkpoints/
│   ├── mpnn_best.pt  # ← download (see below)
│   └── egnn_best.pt  # ← download (see below)
├── outputs/
│   ├── mpnn_log.csv  # ← download (training logs for Fig 1)
│   └── egnn_log.csv
└── README.md
```
...
Code generation and debugging were assisted by Claude (Anthropic). All experimental
design, model architecture decisions, hyperparameter choices, result interpretation,
and report writing are entirely my own.
